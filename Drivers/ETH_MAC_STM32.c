...
__Conceptual__ deviations:
  - the STM32CubeMX generated initialization code (function MX_ETH_Init) will already
...
__Functional__ deviations:
  ...
  - GetTxFrameTime:
    - not supported by HAL
  - Control:
    - functionality for control codes ARM_ETH_MAC_CONTROL_TX, ARM_ETH_MAC_FLUSH and ARM_ETH_MAC_SLEEP is not supported by HAL
    - VLAN filter disable for control code ARM_ETH_MAC_VLAN_FILTER is not supported by HAL
  - ControlTimer:
    - not supported by HAL
...
This is example of configuring **ETH** on the **STM32H735IGK3** device with the **STM32CubeMX** tool.
...
Results of the **CMSIS-Driver Validation** for this driver can be found in the [ETH_MAC_TestReport.txt](../../Drivers/Validation/ETH_MAC_TestReport.txt) file.
