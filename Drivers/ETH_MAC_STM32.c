...
__Functional__ deviations:
  - GetCapabilities:
    - relies on code generated by CubeMX.
  - Initialize:
    - relies on code generated by CubeMX.
    - does not initialize GPIO pins.
  - Uninitialize:
    - does not deinitialize GPIO pins.
  - PowerControl:
    - relies on code generated by CubeMX.
    - low-power mode is not supported by HAL.
  - SetAddressFilter:
    - supported on newer STM32H and STM32N devices.
    - not supported on older STM32F devices.
  - GetRxFrameTime:
    - not supported by HAL.
  - GetTxFrameTime:
    - not supported by HAL.
  - Control:
    - control codes not supported: `ARM_ETH_MAC_CONTROL_TX`, `ARM_ETH_MAC_FLUSH`, and `ARM_ETH_MAC_SLEEP`.
    - `ARM_ETH_MAC_VLAN_FILTER` control code supported on newer STM32H and STM32N devices.
  - ControlTimer:
    - not supported by HAL.
...
>   are located in **non-cacheable** and **shareable** device memory.
...
If using the GNU Compiler or Arm Compiler 6, add the `RxDescripSection`, `TxDescripSection`, `driver.eth_mac0_rx_buf`,
and `driver.eth_mac0_tx_buf` sections to the scatter file.
...
RW_ETH_RX_BUF  0x30040100 0x00001E00 {
  *(.driver.eth_mac0_rx_buf)
}
RW_ETH_TX_BUF  0x30041F00 0x00001800 {
  *(.driver.eth_mac0_tx_buf)
}
~~~

In older HAL versions, the DMA descriptor sections were named `RxDecripSection` and `TxDecripSection`.
*/
...
// EMAC Tx buffer owners
#define TX_BUF_OWNER_APP                0U
#define TX_BUF_OWNER_DMA                1U
...
typedef struct {
  ...
  struct {
    ...
    uint8_t                     owner[EMAC_TX_BUF_CNT]; // Tx buffer owner
  } tx;
...
static int32_t ETH_MAC_SendFrame (const uint8_t *frame, uint32_t len, uint32_t flags) {
  ...
  if (eth_mac0_rw_info.tx.length == 0U) {
    ...
    if (eth_mac0_rw_info.tx.owner[eth_mac0_rw_info.tx.write_idx] != TX_BUF_OWNER_APP) {
      // Transmitter is busy, all buffers in use
  ...
  eth_mac0_rw_info.tx.owner[eth_mac0_rw_info.tx.write_idx] = TX_BUF_OWNER_DMA;
...
void HAL_ETH_TxCpltCallback(ETH_HandleTypeDef *h_eth) {
  ...
  eth_mac0_rw_info.tx.owner[eth_mac0_rw_info.tx.consume_idx] = TX_BUF_OWNER_APP;
